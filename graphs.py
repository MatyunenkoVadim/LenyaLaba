...
def show_graphs():
    ...
    kr_universe = np.arange(0, 6, 1)
    class_work_universe = np.arange(0, 16, 1)

    # Определение функций принадлежности для посещаемости
    attendance_normal = fuzz.trimf(attendance_universe, [0, 100, 100])
    attendance_high = fuzz.trimf(attendance_universe, [75, 100, 100])
